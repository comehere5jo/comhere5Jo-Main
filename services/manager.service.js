...
class ManagerService {
  ...
  getOrderReview = async (orderId) => {
    const getOrderReview = await this.reviewRepository.findReviewOrderId(
      orderId,
    );
    return {
  ...
  getMyOrderReview = async (managerId) => {
    managerId = 1;
    const getOrderReview = await this.reviewRepository.findReviewManagerId(
      managerId,
    );
  ...
  updateOrder = async (orderId, managerId, status) => {
    ...
    try {
      if (myOrder.status === 1) {
        ...
        const updateOrder = await this.orderRepository.startOrder(
          orderId,
          status,
        );
  ...
  managerSignin = async (loginId, loginPw) => {
    try {
      const manager = await this.managerRepository.findCertainManager(loginId);
